...
function M.run(close_floating_window)
  ...
  vim.ui.select(parsed_table_names, { prompt = "Removing reference from which table?" }, function(selected_table)
    if selected_table ~= nil then
      ...
      vim.ui.select(
        parsed_model_names,
        { prompt = "Removing reference of which model?" },
        function (selected_model)
          if selected_model ~= nil then
            local nvim_notify_ok, nvim_notify = pcall(require, 'notify')

            local migration_name = "remove_" .. selected_model .. "_from_" .. selected_table
